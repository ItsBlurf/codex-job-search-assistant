...
<!-- SETUP: Profile statements and section ordering are personalized by the job-setup skill. -->
...
- **Reading order.** The stock banking style is single-column, so extraction order matches visual order. Custom templates added with the `add-template` skill can interleave unrelated lines when they use sidebars or multi-column layouts; if extraction order is scrambled, the user is trading ATS compatibility for looks and should be told.
